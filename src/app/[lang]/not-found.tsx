...
import { getContent } from "@/features/landing/content";
import type { Locale } from "@/features/landing/types";

export default function NotFound() {
  const pathname = usePathname();
  // not-found boundaries receive no params, so the locale comes from the URL
  const locale: Locale = pathname === "/es" || pathname.startsWith("/es/") ? "es" : "en";
  const content = getContent(locale);

  return (
    <div className="flex min-h-full flex-1 flex-col">
      <SiteHeader locale={locale} brand={content.brand} />
